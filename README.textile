...
Then, once spork is running, invoke @testdrb@ (e.g. @testdrb -I test test/your_test.rb@) to run your tests under Spork. To use a port other than the default simply add @-p 1234@ or @--port 1234@ to you command (where @1234@ is the port number that Spork is running on).
